...
SECURITY_DIR = os.path.dirname(os.path.realpath(__file__))
...
def main(argv):
    ...
    tool_excutable = os.path.join(SECURITY_DIR, "run-dljc.sh")

    corpus_name = args.corpus if not args.corpus == None else os.path.splitext(args.corpus_file)[0]

    BENCHMARK_DIR = os.path.join(SECURITY_DIR, corpus_name)
    ...
    projects = None
    with open (os.path.join(SECURITY_DIR, args.corpus_file)) as projects_file:
    ...
    print "----- Running Security on corpus... -----"
    ...
    print "----- Running Security on corpus done. -----"
